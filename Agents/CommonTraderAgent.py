...
class CommonTraderAgent(BaseAgent):
    ...
    def make_decision(self):
        ...
        if (self.model.nprandom.binomial(n=1, p=self._sell_probability) == 1 or self._money < 0) and self._inventory > 0:
            ...
            order_size = self.model.nprandom.uniform(high=self._risk_level) * self._inventory
        else:
            if order_price == 0:
                return
            order_type = OperationTypes.BUY
            order_size = self.model.nprandom.uniform(high=self._risk_level) * self._money / order_price
