...
class MarketMakerAgent(BaseAgent):
    ...
    def make_decision(self):
        ...
        if -self._risk_level <= self._inventory <= self._risk_level:
            ...
            if abs(abs(self._inventory) - self._risk_level) > 1:
                print('FAIL')
            if self._inventory < 0:
                market_env.add_order(1e20, abs(self._inventory) - self._risk_level * 0.99, OperationTypes.BUY, self, report=self.p.report)
            else:
                market_env.add_order(0, abs(self._inventory) - self._risk_level * 0.99, OperationTypes.SELL, self, report=self.p.report)
        return
